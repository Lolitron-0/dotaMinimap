const SELECTED_ROW_STYLE =
  "--bs-table-bg: black;  --bs-table-hover-bg: black; --bs-table-color: white; --bs-table-hover-color: white; border-left: 3px white solid; ";
const EPSILON = 1e-9;
...
class Rectangle extends IDrawable {
  ...
  draw(cx) {
    cx.lineWidth = 2;
    cx.strokeStyle = "red";
    cx.strokeRect(this.position.x, this.position.y, this.width, this.height);
  }
...
//a class to store and swap two values (e. g. hover and normal size)
class DoubleValue {
  ...
  swap() {
    if (this.current == this.first) this.setSecond();
    else this.setFirst();
  }
