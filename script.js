let players = [];
let lastSelectedPlayerMode = PlayerInteractionMode.AREAS;
let trees = [];
...
Array.from(document.getElementsByClassName("tool-button")).forEach((button) => {
	button.onclick = function () {
		...
		players.forEach((player) => {
			player.switchTo(PlayerInteractionMode.NONE);
		});
		...
		switch (button.id) {
			...
			case "areamode":
				lastSelectedPlayerMode = PlayerInteractionMode.AREAS;
				playerTable.setCheckedPlayerIndex(playerTable.checkedIndex);
				break;

			case "speedmode":
				lastSelectedPlayerMode = PlayerInteractionMode.MS; //huinya ya zaputalsya
				playerTable.setCheckedPlayerIndex(playerTable.checkedIndex);
				break;
...
playerTable.onselectionchanged = () => {
	...
	players[playerTable.checkedIndex].switchTo(lastSelectedPlayerMode);
};
